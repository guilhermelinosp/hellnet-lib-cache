...
func New(ctx context.Context, ops telemetry.Client) (*HybridCache, error) {
	...
	o := Options{
		...
		L1DefaultTTL:              environments.GetDuration("", "", "CACHE_L1_DEFAULT_TTL", 5*time.Minute),
		L1ExpirationScanFrequency: environments.GetDuration("", "", "CACHE_L1_EXPIRATION_SCAN_FREQUENCY", time.Minute),
		...
		ConnectTimeout:            environments.GetDuration("", "", "CACHE_CONNECT_TIMEOUT", 5*time.Second),
		ReadTimeout:               environments.GetDuration("", "", "CACHE_SYNC_TIMEOUT", time.Second),
		RetryCount:                environments.GetInt("", "", "CACHE_RETRY_COUNT", 2),
		RetryBaseDelay:            environments.GetDuration("", "", "CACHE_RETRY_BASE_DELAY_MS", 200*time.Millisecond),
		CircuitBreakerFailures:    environments.GetInt("", "", "CACHE_CB_FAILURES", 5),
		CircuitBreakerDuration:    environments.GetDuration("", "", "CACHE_CB_DURATION_SEC", 30*time.Second),
		...
		DefaultTTL:                environments.GetDuration("", "", "CACHE_DEFAULT_TTL", 30*time.Minute),
		MaxTTL:                    environments.GetDuration("", "", "CACHE_MAX_TTL", 24*time.Hour),
		TouchOnRead:               environments.GetBool("", "", "CACHE_TOUCH_ON_READ", false),
		TouchTTL:                  environments.GetDuration("", "", "CACHE_TOUCH_TTL", 10*time.Minute),
	}
